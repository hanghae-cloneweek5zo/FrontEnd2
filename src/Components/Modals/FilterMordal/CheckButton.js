      ...
      <CheckButtonArea>
        ...
        <CheckButtonLine>
          ...
          <AnyButton
            key="bedroom"
            onClick={() => {
              roomNum === 0 ? setRoomNum(7) : setRoomNum(0);
            }}
            check={0}
            num={roomNum}
          >
          ...
          {List.map((item, index) => {
            return (
              <CheckButton
                key={item + 'room'}
                onClick={() => {
                  item === roomNum ? setRoomNum(7) : setRoomNum(item);
                }}
                check={item}
                num={roomNum}
              >
        ...
        <CheckButtonLine>
          ...
          <AnyButton
            ...
            num={bedNum}
            onClick={() => {
              bedNum === 0 ? setBedNum(7) : setBedNum(0);
            }}
          >
            상관없음
          </AnyButton>
          {List.map((item, index) => {
            return (
              <CheckButton
                key={item + 'bed'}
                onClick={() => {
                  item === bedNum ? setBedNum(7) : setBedNum(item);
                }}
                check={item}
                num={bedNum}
              >
                {index === List.length ? item + '+' : item}
              </CheckButton>
            );
          })}
        </CheckButtonLine>
